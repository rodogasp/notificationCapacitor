...
public final class MediaPlaybackPlugin extends Plugin {
    ...
    @PluginMethod public void selectMediaFile(PluginCall call) { pending = call; Intent intent = new Intent(Intent.ACTION_OPEN_DOCUMENT).setType("video/*").addCategory(Intent.CATEGORY_OPENABLE).addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION); startActivityForResult(call, intent, "selected"); }
    ...
    @Override protected void handleOnActivityResult(int requestCode, int resultCode, Intent data) { PluginCall call = pending; pending = null; if (call == null || resultCode != Activity.RESULT_OK || data == null || data.getData() == null) { if (call != null) call.reject("No video selected"); return; } Uri uri = data.getData(); getContext().getContentResolver().takePersistableUriPermission(uri, Intent.FLAG_GRANT_READ_URI_PERMISSION); MediaPlaybackService.setUri(uri.toString()); call.resolve(new JSObject().put("name", uri.getLastPathSegment()).put("uri", uri.toString())); }
}
